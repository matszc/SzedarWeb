...
const Bracket = ({upper, final, lower, gameClick}) => {
    ...
    const size = lower ? {
            ...
            height: 100 * (upperHeight + (upperHeight / 2)) + 100
        } :
        {
    ...
    return (
        <DragScroll height={window.innerHeight * 0.9} width={window.innerWidth * 0.8 - 48} className={style.container}>
            <svg height={size.height} width={size.width}>
                <g>
                    ...
                    {lower ?
                        <rect x={0} y={upperHeight * 100 - 20} width={size.width} stroke={'#999999'} strokeWidth={'2'}
